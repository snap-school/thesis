...
Deux outils principaux ont été utilisés pour avoir un logiciel le mieux écrit possible. Cette section met en pratique \ref{rails-tests}.
%TODO faire intro

\subsection{Tests unitaires}
Les tests unitaires servent à vérifier que le code testé fonctionne comme il doit. L'implémentation des tests c'est déroulée en parallèle de la création de l'application. Durant la conception de Rsnap, deux principaux types de tests ont été réalisés.

\subsubsection{Outils pour les tests}
Pour réaliser ces tests, Travis-CI (\ref{travis}) est relié à Github pour que tout les tests soient exécuté à chaque commit. De plus, lors des "pull request" un message est affiché pour connaitre l'état de la branche. Le programmeur peut donc facilement connaitre si l'acceptation de la branche introduira des bugs.

Une métrique générée par les tests est le couverture du code par ceux-ci. Cette information est fournie par Coveralls\ref{biblio} qui est liée à Travis-CI pour l'aquisition des données. Cette métrique est utile pour connaitre si le code est suffisament testé. Elle ne donne par contre pas l'information du type de code testé, il faut donc faire attention à ce que ce ne soit pas uniquement du code trivial qui soit testé.

Des données de tests sont aussi nécessaire. Celle-ci sont fournie grâce au gem FactoryGirls \footnote{\url{https://github.com/thoughtbot/factory_girl_rails}} qui propose un langage dédié pour écrire rapidement des fabriques.

\subsubsection{Tests au niveau des controlleurs}
Une première batterie de tests réalisée grâce à RSpec. Ces tests ont pour vocations de vérifier l'accès aux différentes routes : les droits des utilisateurs et le type de retour des contrôleurs.
